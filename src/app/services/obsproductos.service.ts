import { Injectable } from '@angular/core';
import { Productos } from '../Models/productos';
import { BehaviorSubject, Observable, map } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class ObsproductosService {

  private obsproductos: Productos[] = [

    {
      id: 1,
      name: "Polera Amarilla",
      categoria: "Ropa",
      stock: 21,
      precio: 17500,
      imagen: "data:image/jpeg;base64,",
      fechaIngreso: new Date(3-10-2022)
    },

    {
      id: 2,
      name: "Zapatillas Nike",
      categoria: "Calzado",
      stock: 33,
      precio: 79000,
      imagen: "https://www.theline.cl/img/products/DM9922-200-1_1024.png",
      fechaIngreso: new Date(5-15-2023)
    },

    {
      id: 3,
      name: "Pelota Addidas",
      categoria: "Deporte",
      stock: 25,
      precio: 39700,
      imagen: "https://assets.adidas.com/images/w_600,f_auto,q_auto/674bd9cb2b044c7cb6b8aaed00eec2c4_9366/Pelota_Entrenamiento_Futsal_Uniforia_UNISEX_Blanco_FH7349_01_standard.jpg",
      fechaIngreso: new Date(8-25-2021)
    },

    {
      id: 4,
      name: "Short Underarmour",
      categoria: "Ropa",
      stock: 52,
      precio: 19800,
      imagen: "https://falabella.scene7.com/is/image/Falabella/14882551_1?wid=800&hei=800&qlt=70",
      fechaIngreso: new Date(11-15-2020)
    },

    {
      id: 5,
      name: "Zapatillas Puma",
      categoria: "Calzado",
      stock: 28,
      precio: 39770,
      imagen: "https://images.puma.com/image/upload/f_auto,q_auto,b_rgb:fafafa,w_2000,h_2000/global/390984/01/sv01/fnd/EEA/fmt/png/Zapatillas-Smash-3.0",
      fechaIngreso: new Date(12-18-2022)
    },

    {
      id: 6,
      name: "Raqueta Tennis",
      categoria: "Deporte",
      stock: 18,
      precio: 129770,
      imagen: "https://wilsonstore.cl/cdn/shop/products/WILWR079810U3_0.jpg?v=1648491400",
      fechaIngreso: new Date(11-15-2020)
    }
  ];

  /*usamos behavior en vez de subject para poder mandar un 
  valor por defecto en el constructor ths.obsproductos */
  private productosSubject: BehaviorSubject<Productos[]>

  constructor() {
    this.productosSubject = new BehaviorSubject<Productos[]>(this.obsproductos)
  }

  //Obtener productos
  obtenerProductos(): Observable<Productos[]> {
    //le enviamos el observable que esta dentro del subject
    return this.productosSubject.asObservable()
  }

  obtenerProducto(id: number): Observable<Productos[]>{
    //usamos el metodo de arriba para luego filtrarlos por id
    return this.obtenerProductos().pipe(
    map((obsproductos: Productos[] ) => 
    obsproductos.filter((obsproductos: Productos) => obsproductos.id === id)
    ))}

  agregarProducto(producto: Productos){
    this.obsproductos.push(producto);

    //Para actualizar la info
    this.productosSubject.next(this.obsproductos)
  }  

  editarProducto(producto: Productos){
    //obtener el id del producto a editar usando findIndex
    let indice = this.obsproductos.findIndex(
      (p: Productos) => p.id === producto.id)

    if (indice > -1) {
      this.obsproductos[indice] = producto
    }  
    //Para actualizar la info
    this.productosSubject.next(this.obsproductos)
  }

  eliminarProducto(id: number){
     //obtener el id del producto a eliminar usando findIndex
     let indice = this.obsproductos.findIndex(
      (p: Productos) => p.id === id)

    if (indice > -1) {
  //usamos slice para eliminar prod empezamos en indice y elimin 1
      this.obsproductos.slice(indice, 1)
    }  
    //Para actualizar la info de los productos
    this.productosSubject.next(this.obsproductos)
  }

}
